...
| M6 | 结果与调整 | 结果页、封顶、三层结果、临时规则、人工番型调整 |
| M7 | 听牌与弃牌分析 | Wait、Discard-to-ready、Worker、排序 |
| M8 | 规则百科 | 番表、番型详情、来源、示例、深链接 |
| M9 | 本地数据 | Saved Examples、Draft、Trash、Undo、多标签保护 |
| M10 | 分享与导入导出 | Copy、Share、Backup、Import、Rule Snapshot |
| M11 | PWA 与更新 | Offline、规则缓存、App/Rule Update |
| M12 | 质量收口 | A11y、跨端、性能、安全、迁移、规则测试 |
...
为大众麻将 v0.1.0 的番型逐个实现 Recognizer。
...
用户已经可以完整录入一副牌，并让 M4 Engine 得到正确输入。
...
刷新、重新打开浏览器、打开已保存牌例等关键场景不会静默覆盖或丢失用户数据。
...
所有导入/导出 roundtrip 测试通过，失败不会破坏现有数据。
...
T4xx  Pattern / Score
T5xx  Calculator UI
T6xx  Result / Adjustment
T7xx  Ready Analysis
T8xx  Encyclopedia
T9xx  Persistence
T10xx Share / Import
T11xx PWA / Update
T12xx Quality / Release
```
