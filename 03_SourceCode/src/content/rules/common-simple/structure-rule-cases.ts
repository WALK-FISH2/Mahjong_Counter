import type { HandSnapshotInput } from '../../../domain/mahjong/hand.ts';
import type { RuleRef } from '../../../domain/mahjong/calculator-document.ts';
import type { WinContext } from '../../../domain/mahjong/context.ts';
import type { StructureKey } from '../../../domain/rules/structure-definition.ts';
...
const DEFAULT_CONTEXT: WinContext = Object.freeze({
  mode: 'discard',
  values: Object.freeze({
    seatWind: Object.freeze({ status: 'known', value: 'east' }),
    roundWind: Object.freeze({ status: 'known', value: 'south' }),
  }),
});

/** Shared Rule Case fixtures consumed by both verification and encyclopedia examples. */
export const COMMON_SIMPLE_STRUCTURE_RULE_CASES = Object.freeze([
  ...
  {
    id: 'encyclopedia-big-four-winds-relation',
    ruleRef: RULE_REF,
    title: '包含与不重复：大四喜覆盖三风刻等下位番型',
    tags: ['encyclopedia', 'relation', 'combination', 'positive'],
    calculator: {
      hand: {
        concealed: ['red'],
        melds: [
          { id: 'east-pung', type: 'pung', tile: 'east' },
          { id: 'south-pung', type: 'pung', tile: 'south' },
          { id: 'west-pung', type: 'pung', tile: 'west' },
          { id: 'north-pung', type: 'pung', tile: 'north' },
        ],
        winningTile: 'red',
      },
      context: {
    ...
    expected: {
      ...
      minimumDecompositionCount: 1,
      minimumWinningTilePlacementCount: 1,
      countedPatternIds: ['bigFourWinds'],
      excludedPatternIds: ['allPungs', 'prevalentWind', 'seatWind'],
      description: '大四喜计入；碰碰和、圈风刻和门风刻被包含关系排除，不重复计番。',
    },
    sourceRefs: SOURCE_REFS,
  },
  {
    id: 'encyclopedia-local-special-boundary',
    ruleRef: RULE_REF,
    title: '地方特殊边界：平台加倍不属于本预设结果',
    tags: ['encyclopedia', 'local-special', 'scope-boundary'],
    calculator: {
      hand: {
        concealed: [
          'm1',
    ...
    expected: {
      ...
      minimumDecompositionCount: 1,
      minimumWinningTilePlacementCount: 1,
      description: '只按当前 RulePackage 计算；庄家、房间倍数和平台奖励不进入结果。',
    },
    sourceRefs: SOURCE_REFS,
  },
